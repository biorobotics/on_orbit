...
from collision.numba_triangle_geometry import stable_triangle_area,point_to_triangle_squared_distance,brute_force_point2mesh_cpu,closest_point_on_triangle
from collision.Math import cuda_vec3_minus
from collision import priority_queue
...
array_rsstree=namedtuple("array_rsstree",["is_leaf","node_to_triangles","triangle_index_array","children","rotation_matrices","centers","half_lengths","radii"])
...
@njit
def rsstree_from_arrays(rsstree_as_arrays:array_rsstree):
    nodes=List()
    n_nodes=len(rsstree_as_arrays.is_leaf)
    node_id=0
    parent=-1
    depth=1
    current=(node_id,parent,depth)
    work=[current]
    remaining=1
    index=0
    while remaining>0:
        node_id,parent,depth=work[index]
        index+=1
        remaining-=1

        start=rsstree_as_arrays.node_to_triangles[node_id]
        end=rsstree_as_arrays.node_to_triangles[node_id+1]
        triangles=rsstree_as_arrays.triangle_index_array[start:end]
        child_minus=rsstree_as_arrays.children[node_id,0]
        child_plus=rsstree_as_arrays.children[node_id,1]
        is_leaf=rsstree_as_arrays.is_leaf[node_id]

        rotation_matrix=rsstree_as_arrays.rotation_matrices[node_id]
        center=rsstree_as_arrays.centers[node_id]
        half_lengths=rsstree_as_arrays.half_lengths[node_id]
        radius=rsstree_as_arrays.radii[node_id]
        RSS=rss(rotation_matrix,center,half_lengths,radius)

        node=rssnode(triangles,parent,child_plus,child_minus,None,None,RSS,depth,is_leaf)

        nodes.append(node)

        if child_minus>=0:
            cminus=(child_minus,node_id,depth+1)
            work.append(cminus)
            remaining+=1
        if child_plus>=0:
            cplus=(child_plus,node_id,depth+1)
            work.append(cplus)
            remaining+=1
    return List(nodes)
...
def get_center_and_split_axes(triangles):
    ...
    triangle areas less than eps=1e-6 are ignored when rescaling Areas
    '''
    eps=1e-6
    n=len(triangles)
    areas=np.array([stable_triangle_area(tri) for tri in triangles],dtype=triangles.dtype)
    Amin=np.nanmin(areas)
    Amin=max(Amin,eps)
    total_area=np.nansum(areas)
    weighted_vertices=np.sum(triangles,1)*np.expand_dims(areas,1)
    centroid=np.empty(3,dtype=triangles.dtype)
    for i in range(3):
        centroid[i]=np.nansum(weighted_vertices[:,i])/total_area/3#np.nansum can't be told to operate along just 1 axis so have to loop manually
...
def triangle_inertia_tensor_contrib(triangle,area,centroid):
    if np.isnan(area):
        return np.zeros((3,3),dtype=triangle.dtype)
...
def fit_RSS(triangles,axes):
    ...
    R=np.empty((3,3),dtype=triangles.dtype)
    R[:,0]=axes[0]
    R[:,1]=axes[1]
    ...
    c=np.array([(maxx+minx)/2,(maxy+miny)/2,cz],dtype=triangles.dtype)
    rectangle_center=R@c
    rectangle_sides=np.array([maxx-minx,maxy-miny],dtype=triangles.dtype)
...
def IPS_split_rule(parent_idx,parent_node,triangle_vertices,leaf_size):
    ...
    for ax in parent_node.axes:
        ...
        if len(plus_tri)>0:
            ...
            plus_centroid=np.full(3,np.nan,dtype=dtype)
            plus_axes=np.full((3,3),np.nan,dtype=dtype)
            plus_rss=rss(np.eye(3,dtype=dtype),np.full(3,np.nan,dtype=dtype),np.zeros(3,dtype=dtype),0.0)
            not_split=True
        ...
        if len(minus_tri)>0:
            ...
            minus_centroid=np.full(3,np.nan,dtype=dtype)
            minus_axes=np.full((3,3),np.nan,dtype=dtype)
            minus_rss=rss(np.eye(3,dtype=dtype),np.full(3,np.nan,dtype=dtype),np.zeros(3,dtype=dtype),0.0)
            not_split=True
        if not_split:
            cost=np.inf
        else:
...
@njit
def sdf_queue(query,rsstree,triangle_vertices,normals):
    best_surface_to_query=np.empty_like(query)
    closest_triangle_id=-1
    best_distance_squared=np.inf
    count=0
    queue=priority_queue.make_queue(0,0.0)
    while not priority_queue.is_empty(queue):
        ...
        if not node.is_leaf:
            ...
            for triangle_id in node.triangles:
                triangle=triangle_vertices[triangle_id]
                point_on_triangle=np.array(closest_point_on_triangle(triangle[0],triangle[1],triangle[2],query,tol),dtype=query.dtype)
                surface_to_query=query-point_on_triangle
                tri_dist_squared=np.sum(surface_to_query*surface_to_query)
                count+=1
                if tri_dist_squared<best_distance_squared:
                    best_distance_squared=tri_dist_squared
                    closest_triangle_id=triangle_id
                    best_surface_to_query=surface_to_query
    sign=np.dot(normals[closest_triangle_id],best_surface_to_query)
    sd=np.sign(sign)*sqrt(best_distance_squared)
    return sd

@njit
def sdf_and_surface_vector_queue(query,rsstree,triangle_vertices,normals):
    best_surface_to_query=np.empty_like(query)
    closest_triangle_id=-1
    best_distance_squared=np.inf
    count=0
    ...
    while not priority_queue.is_empty(queue):
        ...
        if not node.is_leaf:
            ...
            for triangle_id in node.triangles:
                triangle=triangle_vertices[triangle_id]
                point_on_triangle=np.array(closest_point_on_triangle(triangle[0],triangle[1],triangle[2],query,tol),query.dtype)
                surface_to_query=query-point_on_triangle
                tri_dist_squared=np.sum(surface_to_query*surface_to_query)
                count+=1
                if tri_dist_squared<best_distance_squared:
    ...
    sign=np.dot(normals[closest_triangle_id],best_surface_to_query)
    sd=np.sign(sign)*sqrt(best_distance_squared)
    return sd,best_surface_to_query

@njit
def get_distance_stack(query,rsstree,triangle_vertices):
    best_distance_squared=np.inf
    count=0
    queue=List([(0,0.0)])
    while len(queue)>0:
        nodeid,lower_bound=queue.pop()
        node=rsstree[nodeid]
        if best_distance_squared<lower_bound:
        ...
        if not node.is_leaf:
            ...
            if node.child_plus>=0:
                distance_to_plus_rss=point_rss_squared_distance(query,rsstree[node.child_plus].RSS)
                if distance_to_plus_rss<best_distance_squared:
                    queue.append((node.child_plus,distance_to_plus_rss))
            if node.child_minus>=0:
                distance_to_minus_rss=point_rss_squared_distance(query,rsstree[node.child_minus].RSS)
                if distance_to_minus_rss<best_distance_squared:
                    queue.append((node.child_minus,distance_to_minus_rss))
        else:
            #leaf node, which may be empty
            #get a candidate distance as the nearest triangle in the leaf node
            tol=1e-12
...
@njit
def is_distance_lte_array(query,rsstree_as_arrays:array_rsstree,triangle_vertices,upper_bound_squared):
    '''
    returns if the distance from a query to the mesh is <= a provided upper bound, with rsstree provided as an array_rsstree

    Parameters: query : (3,) float array
                    the point to test
                rsstree_as_arrays : array_rsstree
                    the RSStree data structure
                triangle_vertices : (n,3,3) float array
                    the vertices of each triangle in the mesh
                upper_bound_squared : float
        ...
        if not rsstree_as_arrays.is_leaf[nodeid]:
            ...
            if rsstree_as_arrays.children[nodeid,1]>=0:
                ...
                distance_to_plus_rss=cuda_point_rss_squared_distance(query,rsstree_as_arrays.rotation_matrices[cid],
                                                                            ...
                                                                            rsstree_as_arrays.radii[cid])
                if distance_to_plus_rss<best_distance_squared:
                    queue=priority_queue.add_item(queue,cid,distance_to_plus_rss)
            ...
            if rsstree_as_arrays.children[nodeid,0]>=0:
                ...
                distance_to_minus_rss=cuda_point_rss_squared_distance(query,rsstree_as_arrays.rotation_matrices[cid],
                                                                            ...
                                                                            rsstree_as_arrays.radii[cid])
                if distance_to_minus_rss<best_distance_squared:
                    queue=priority_queue.add_item(queue,cid,distance_to_minus_rss)        
        else:
            #leaf node, which may be empty
            #get a candidate distance as the nearest triangle in the leaf node
            tol=1e-12
...
@njit(parallel=True)
def sdf_via_rss(points,rsstree,triangle_vertices,normals):
    distances=np.empty(len(points),dtype=points.dtype)
    for i in prange(len(points)):
        distances[i]=sdf_queue(points[i],rsstree,triangle_vertices,normals)
    return distances

@njit(parallel=True)
def sdf_and_surface_via_rss(points,rsstree,triangle_vertices,normals):
    distances=np.empty(len(points),dtype=points.dtype)
    surface_to_query=np.empty((len(points),3),dtype=points.dtype)
    for i in prange(len(points)):
        distances[i],surface_to_query[i]=sdf_and_surface_vector_queue(points[i],rsstree,triangle_vertices,normals)
    return distances,surface_to_query
