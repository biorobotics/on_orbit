...
class IpoptContactAlignPlanner(object):
  ...
  def plan(self, x0, ecm_sim, ecm_sim_reset_args, save_path=None, max_iter=250, count_flop_per_iter=False, count_total_flop=False, stop_after_iter=-1, prop_time=0.):
    ...
    #### PHASE 5: ENFORCE-ALIGNMENT
    #### PHASE 6: ENFORCE VELOCITY-CONSTRAINT (dummy)
    ...
    phase_lengths = np.array([nondummy_phase_lengths[0], 
                              nondummy_phase_lengths[1], 
                              1, 
                              nondummy_phase_lengths[2], 
                              nondummy_phase_lengths[3],
                              nondummy_phase_lengths[4], 
                              1])
    ...
    if self.apply_joint_acc_constraint: 
        h_expr, h_lb, h_ub = IpoptContactAlignPlanner.append_joint_acc_constraints(h_expr, h_lb, h_ub, self.cpin_model, constraint_input, self.joint_acc_limits, self.dt)
    ...
    h2_lb[0] = 0.15 # We modify the bounds of the stay-below-nozzle to make it a stay-above-nozzle constraint
    h2_ub[0] = inf
    ...
    # h_expr = ca.vertcat(dynamics_expr, h2_expr, h3_expr, h4_expr)
    # h_lb = np.concatenate((dynamics_lb, h2_lb, h3_lb, h4_lb))
    # h_ub = np.concatenate((dynamics_ub, h2_ub, h3_ub, h4_ub))
    ...
    # Extened area of alignment to guide the arm to final alignment
    position_upper_tol = np.array([1, 1, 0.3])
    position_lower_tol = -position_upper_tol
    
    rotation_upper_tol = np.array([0.5,0.5,0.5])
    ...
    #### PHASE 5: Enforce Period of Alignment ####
    phase_idx += 1
    ...
    # Extened area of alignment to guide the arm to final alignment
    position_upper_tol = np.array([0.001, 0.001, 0.01])
    position_lower_tol = -position_upper_tol
    
    rotation_upper_tol = np.array([0.1,0.1,0.10])
    rotation_lower_tol = -rotation_upper_tol
    ...
    #### PHASE 6: Enforce Velocity Constraints ####
    phase_idx += 1
