...
class CasadiExternalFunction3 {
  public:
    CasadiExternalFunction3(const std::string &function_name) {
      std::cout << "Opening " << function_name << std::endl;
      /* Load the dll */
      handle = dlopen((function_name + ".so").c_str(), RTLD_LAZY);
      if(handle==0){
    ...
    ~CasadiExternalFunction3() {
      /* Free the handle */
      dlclose(handle);
    }

    CasadiExternalFunction3 (const CasadiExternalFunction3 &) = delete;
    CasadiExternalFunction3 & operator = (const CasadiExternalFunction3 &) = delete;
...
int main() {
  CasadiExternalFunction3 control_fn("control");
  double x[103];
  for (int i = 0; i < 103; ++i) {
    x[i] = i;
  }
  ...
  double joint_acc_cmd[7] = {1,2,3,4,5,6,7};
  std::cout << "process id is " << getpid() << std::endl;
  std::cout << "calling function lots of times after 10 s" << std::endl;
  sleep(10);
  double num_loop = 1000000;
  std::cout << "Calling function" << std::endl;

  auto start = high_resolution_clock::now();
  for (int i = 0; i < num_loop; ++i) 
  {
    control_fn.call(x, joint_acc_cmd);
  }

  auto stop = high_resolution_clock::now();
  auto duration = duration_cast<microseconds>(stop - start);
  std::cout << "Seconds: " << duration.count()/1000000. << std::endl;
  //double sec_per_loop = duration.count()/1000000./num_loop;
  //std::cout << "Seconds per loop" << sec_per_loop << std::endl;
