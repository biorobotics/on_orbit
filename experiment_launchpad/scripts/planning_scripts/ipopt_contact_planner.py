...
class IpoptContactPlanner(object):
  def __init__(self, urdf_file, mrv_urdf_file, dt, 
    ...
    self.ipopt_nx = 2*self.pin_model.nv
    ...
    self.R_mat = np.diag(control_cost_weight*np.ones(self.num_rotary + 3))
  ...
  def plan(self, x0, ecm_sim, ecm_sim_reset_args, save_path=None, max_iter=250, count_flop_per_iter=False, count_total_flop=False, stop_after_iter=-1, prop_time=0.):
    ...
    if prop_time != 0.:
      # Propagate the client state forward
      dt = 0.01
      prop_steps = int(prop_time/dt)

      client_pos = np.copy(x0[self.cv_qidx:self.cv_qidx + 3])
      client_rmat = R.from_quat(x0[self.cv_qidx + 3:self.cv_qidx + 7]).as_matrix()
      v0 = x0[self.pin_model.nq:]
      client_v = np.copy(v0[self.cv_vidx:self.cv_vidx + 3])
      client_w = np.copy(v0[self.cv_vidx + 3:self.cv_vidx + 6])

      mrv_q = x0[self.mrv_qidx:self.mrv_qidx + self.mrv_pin_model.nq]
      mrv_v = x0[self.pin_model.nq + self.mrv_vidx:self.pin_model.nq + self.mrv_vidx + self.mrv_pin_model.nv]

      mrv_com = pin.centerOfMass(self.mrv_pin_model, self.mrv_pin_data, mrv_q, mrv_v)
      mrv_vcom = self.mrv_pin_data.vcom[0]

      for step in range(prop_steps):
        # Linear velocity dynamics (CW eqns)
        separation = mrv_com - client_pos
        cw_x = self.cw_xproj@separation 
        cw_y = self.cw_yproj@separation
        cw_z = self.cw_zproj@separation

        vdiff = mrv_vcom - client_rmat@client_v
        ...
        client_vdot = client_rmat.transpose()@cw_force/self.client_mass - np.cross(client_w, client_v)

        # Angular velocity dynamics (Euler's eqns)
        #client_wdot = -self.client_inertia_inv@(np.cross(client_w, self.client_inertia@client_w))

        # Integrate linear velocity and position dynamics (semi-implicit Euler)
        #client_v += client_vdot*dt
        #client_pos += client_v*dt

        # Integrate angular velocity and orientation dynamics (semi-implicit Euler)
        #client_w += client_wdot*dt
        #client_rmat = client_rmat@pin.exp3(client_w*dt)

      x0[self.cv_qidx:self.cv_qidx + 3] = client_pos
      x0[self.cv_qidx + 3:self.cv_qidx + 7] = R.from_matrix(client_rmat).as_quat()
      x0[self.pin_model.nq + self.cv_vidx:self.pin_model.nq + self.cv_vidx + 3] = client_v
      x0[self.pin_model.nq + self.cv_vidx + 3:self.pin_model.nq + self.cv_vidx + 6] = client_w
    ...
    ik_problem = FloatingBaseIKProblem(self.mrv_pin_model, \
                                       ...
                                       self.pin_data.oMf[self.goal_fid].translation, \
                                       self.pin_data.oMf[self.goal_fid].rotation)
    ...
    init_dts = self.dt*np.ones(self.num_phases)
    num_phases_without_dummy = 4 # Dummy phases are the nozzle impact, hole impact, and insertion phases

    #### PHASE 0: STAY-BELOW-NOZZLE-OPENING-PLANE
    #### PHASE 1: STAY-IN-EXTENDED-NOZZLE 
    #### PHASE 2: IMPACT-NOZZLE (dummy)
    ...
    nondummy_phase_lengths = np.divide(nondummy_phase_lengths_sec,self.dt).astype(int) # Number of time instants per phase

    phase_lengths = np.array([nondummy_phase_lengths[0], 
                              nondummy_phase_lengths[1] - 1, 
                              1, 
                              nondummy_phase_lengths[2] - 1, 
                              1, 
                              nondummy_phase_lengths[3], 
                              1]) # Number of intervals per phase
    ...
    if self.apply_joint_acc_constraint: 
        h_expr, h_lb, h_ub = IpoptContactPlanner.append_joint_acc_constraints(h_expr, h_lb, h_ub, self.cpin_model, constraint_input, self.joint_acc_limits, self.dt)
    ...
    # (2) peg direction avoids side contact
    h1_expr, h1_lb, h1_ub = export_stay_in_cone_approx_planes_constraint(self.cpin_model, x_sym, self.cone_slope)
    h2_expr, h2_lb, h2_ub = export_peg_direction_constraint(self.cpin_model, x_sym, self.cone_slope)
    # h3_expr, h3_lb, h3_ub = export_contact_force_direction_constraint(self.cpin_model, x_sym, u_sym, plane_idx, np.linalg.norm(self.u_ub[self.num_rotary:self.num_rotary + 3]))
    ...
    # Nonlinear constraints:
    # (1) stay in extended nozzle cone, and on the desired plane (of the planes being used to approximate the nozzle)
    # (2) stay above nozzle opening plane
    # (3) peg direction avoids side contact
    h1_expr, h1_lb, h1_ub = export_stay_in_cone_approx_planes_constraint(self.cpin_model, x_sym, self.cone_slope)
    h1_lb[plane_idx] = 0.
    h2_expr, h2_lb, h2_ub = export_stay_below_nozzle_constraint(self.cpin_model, x_sym)
    h2_lb[0] = 0.015 # We modify the bounds of the stay-below-nozzle to make it a stay-above-nozzle constraint
    h2_ub[0] = inf
    h3_expr, h3_lb, h3_ub = export_peg_direction_constraint(self.cpin_model, x_sym, self.cone_slope)
    h4_expr, h4_lb, h4_ub = export_contact_force_direction_constraint(self.cpin_model, x_sym, u_sym, plane_idx, np.linalg.norm(self.u_ub[self.num_rotary:self.num_rotary + 3]))

    h_expr = ca.vertcat(dynamics_expr, h1_expr, h2_expr, h3_expr, h4_expr)
    h_lb = np.concatenate((dynamics_lb, h1_lb, h2_lb, h3_lb, h4_lb))
    h_ub = np.concatenate((dynamics_ub, h1_ub, h2_ub, h3_ub, h4_ub))
    ...
    # Control limits (we can have joint torques and normal forces wrt nozzle in this phase)
    u_lb = np.copy(self.u_lb)
    u_ub = np.copy(self.u_ub)
    ...
    u_lb[self.num_rotary:self.num_rotary + 3] = 0
    u_ub[self.num_rotary:self.num_rotary + 3] = 0
    ipopt_lb[-1][-self.nu:] = u_lb
    ipopt_ub[-1][-self.nu:] = u_ub
    ...
    # Nonlinear constraints:
    # (1) stay in extended nozzle cone
    # (2) peg direction avoids side contact (add this eventually, but in my ipopt version, I commented it out)
    h1_expr, h1_lb, h1_ub = export_stay_in_cone_approx_planes_constraint(self.cpin_model, x_sym, self.cone_slope)
    # h2_expr, h2_lb, h2_ub = export_peg_direction_constraint(self.cpin_model, x_sym)
    # h3_expr, h3_lb, h3_ub = export_contact_force_direction_in_hole_constraint(self.cpin_model, x_sym, u_sym)

    #h4_expr, h4_lb, h4_ub = zero_joint_vel_constraint(self.cpin_model, constraint_input)
    #h_expr = ca.vertcat(dynamics_expr, h1_expr, h4_expr)
    #h_lb = np.concatenate((dynamics_lb, h1_lb, h4_lb))
    #h_ub = np.concatenate((dynamics_ub, h1_ub, h4_ub))
    ...
    # Nonlinear constraints:
    # (1) stay in hole
    ''' If using sliding contact, we don't want to exit the hole once we hit the hole frame.'''
    hole_length = 0.0445
    z_lower_bound = -hole_length
    h1_expr, h1_lb, h1_ub = export_stay_in_hole_constraint(self.cpin_model, x_sym, z_lower_bound)
    # h2_expr, h2_lb, h2_ub = export_contact_force_direction_in_hole_constraint(self.cpin_model, x_sym, u_sym)
    ...
    # Nonlinear constraints:
    # (1) insert
    h1_expr, h1_lb, h1_ub = export_insertion_constraint(self.cpin_model, x_sym)
    ...
    ipopt_lb.append(np.concatenate((x_lb, u_lb)))
    ipopt_ub.append(np.concatenate((x_ub, u_ub)))
    ...
    solver = IPOPTContactSolver(self.ipopt_nx, self.nu, phase_starts, constraint_sizes, ipopt_lb, ipopt_ub, ipopt_cl, ipopt_cu, warm_start, stop_after_iter, plane_idx, max_iter)
