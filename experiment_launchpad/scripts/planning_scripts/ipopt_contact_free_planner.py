...
class IpoptContactFreePlanner(object):
  def __init__(self, urdf_file, 
               mrv_urdf_file, dt, 
               joint_angle_lower_limits, joint_angle_upper_limits, 
               joint_torque_limits, joint_vel_limits, joint_acc_limits, 
               control_cost_weight, phase_lengths_sec, pause_in_nozzle, cw_a, cw_mu, cw_orbit_dir, initial_client_rmat, cone_slope, use_cw, meshdir):
    ...
    self.num_phases = 4
    self.num_phases_without_dummy = 3 # Dummy phases are the nozzle impact, hole impact, and insertion phases
    ...
    self.pause_in_nozzle = pause_in_nozzle
  ...
  def plan(self, x0, ecm_sim, ecm_sim_reset_args, save_path=None, max_iter=250, count_flop_per_iter=False, count_total_flop=False, stop_after_iter=-1, prop_time=0.):
    ...
    if self.apply_joint_acc_constraint: 
      h_expr, h_lb, h_ub = IpoptContactFreePlanner.append_joint_acc_constraints(h_expr, h_lb, h_ub, self.cpin_model, constraint_input, self.joint_acc_limits, self.dt)
    ...
    # At the end of this phase, we should be at the beginning of the hole. 
    phase_idx += 1
    ...
    # Add a constraint so that probe is truly inside in the nozzle in this phase
    #nozzle_rim_to_hole_dist = 0.110 + 0.008
    #z_lower_bound = -nozzle_rim_to_hole_dist
    #h3_expr, h3_lb, h3_ub = export_stay_in_hole_constraint(self.cpin_model, x_sym, z_lower_bound)
    ...
    if self.pause_in_nozzle:
      ### Phase 1B: Require joint velocity and acceleration to be zero ###
      phase_idx += 1

      h1_expr, h1_lb, h1_ub = export_stay_in_cone_approx_planes_constraint(self.cpin_model, x_sym, self.cone_slope)
      h2_expr, h2_lb, h2_ub = zero_joint_vel_constraint(self.cpin_model, constraint_input)

      h_expr = ca.vertcat(dynamics_expr, h1_expr, h2_expr)
      h_lb = np.concatenate((dynamics_lb, h1_lb, h2_lb))
      h_ub = np.concatenate((dynamics_ub, h1_ub, h2_ub))
  
      h_expr, h_lb, h_ub = IpoptContactFreePlanner.append_joint_acc_constraints(h_expr, h_lb, h_ub, self.cpin_model, constraint_input, 0*self.joint_acc_limits, self.dt)
      #if self.apply_joint_acc_constraint: 
      #  h_expr, h_lb, h_ub = IpoptContactFreePlanner.append_joint_acc_constraints(h_expr, h_lb, h_ub, self.cpin_model, constraint_input, self.joint_acc_limits, self.dt)
    ...
    #### PHASE 2: STAY IN HOLE ####
    phase_idx += 1

    # Nonlinear constraints:
    # (1) stay in hole
    hole_length = 0.0445
    nozzle_rim_to_hole_dist = 0.112

    '''We don't want the peg to go past the back of the hole,
    but we also want to be able to set the phase to a longer value and have the robot slow
    down before it actually hits the hole, so we make the lower bound to be n-times the length 
    of the hole. Note the z-axis is pointing into the client. '''

    z_lower_bound = -hole_length - 0.5*(nozzle_rim_to_hole_dist - hole_length)
    ...
    # h_lb = -np.inf*np.ones_like(h1_lb)
    # h_ub = np.inf*np.ones_like(h1_ub)
