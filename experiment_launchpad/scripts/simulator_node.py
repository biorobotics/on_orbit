...
for x in range(0,num_trial):
  for grid_idx in range(initial_grid_idx, final_grid_idx):
    ...
    #interpolate_on_ws = rospy.get_param('interpolate_on_ws')
    #if interpolate_on_ws:
    #  load_paths_for_interpolation,weights = interp_trajectories_on_initial_client_w(load_paths,initial_client_w,ws)
    #else: 
    ...
    # Effectively disables the controller
